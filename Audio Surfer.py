...
def Play():
    ...
    song_path = os.path.join('C:', 'Users', 'lenovo', 'Desktop', 'DataFlair', 'Notepad', 'Music', song_name)
...
def Next():
    ...
    next_one = next_one[0] + 1 if next_one else 0

    # Check if there are more songs in the list
    if next_one < songs_list.size():
        # Get the next song
        temp = songs_list.get(next_one)
        temp_path = os.path.join('C:', 'Users', 'lenovo', 'Desktop', 'DataFlair', 'Notepad', 'Music', temp)

        # Load and play the next song
        mixer.music.load(temp_path)
        mixer.music.play()

        # Clear the selection and set the next song as selected
        songs_list.selection_clear(0, END)
